...
def menu():
    while True:
        print("Menu:")
        print("1. Khởi động luồng mới")
        print("2. Xem danh sách các luồng")
        print("0. Thoát")
        choice = input("Chọn tùy chọn: ")

        if choice == "1":
            thread_id = len(arrThread) + 1
            thread = threading.Thread(target=runtest, args=(thread_id,))
            arrThread.append(thread)
            thread.start()
        elif choice == "2":
            print("Danh sách các luồng đang chạy:")
            for idx, thread in enumerate(arrThread, start=1):
                print(f"Luồng {idx}: {thread.name}")
        elif choice == "0":
            break
        else:
